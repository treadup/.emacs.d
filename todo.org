...
The culprit seems to be the spaceline package. If I comment out the
spaceline package usage then the scrolling issue mostly goes away.

One fix would be to replace spaceline with another package that does not
have an issue with scrolling. Or we could go back to using the default
modeline in Emacs.
